...
#define DATAPIN     24  //green
#define CLOCKPIN    25  //yellow
#define numLED 120
...
volatile uint16_t W1 = 5, W2 = 5, W3 = 12;

#define dbDelay 50
#define dbDelaylong 2000
...
void ConstBlend() {                                                               /* 1 */
  if (millis() - PastMillis > 5) {
    ...
    if (iMain == 0) FirstColor(StepSize/3);
...
void MeteorRain(byte meteorSize, byte meteorTrailDecay, bool meteorRandomDecay) { /* 7 */
  int invrSpd = random(55, 120);

  for (byte i = 0; i < numLED + (meteorTrailDecay/meteorSize);) {
    if (millis() - PastMillis > invrSpd) {
      PastMillis = millis();
      invrSpd = random(40, 80);

      for (byte j = 0; j < numLED; j++) {
        if ( (!meteorRandomDecay) || (random(10) > 6) ) {
      ...
      for (byte j = 0; j < meteorSize; j++) {
        if ( ( i - j < numLED) && (i - j >= 0) ) {
          StripStar.setPixelColor(i - j, W1 << 3, W2 << 3, W3 << 3);
        }
      }
      StripStar.show();
      i++;
...
void fadeToBlack(int ledNo, byte fadeValue) {
  ...
  StripStar.setPixelColor(ledNo, w1 >> 1, w2 >> 1, w3 >> 1);
}
...
void FirstColor(float Step1) {
  if (state1 == 1) {
    (R < MyBrightness - StepHold ? R += Step1 : R = MyBrightness);  // Increase Red
    (B > StepHold ? B -= Step1 : B = 0);                            // Decrease Blue
  }
  else if (state1 == 2) {
    (G < MyBrightness - StepHold ? G += Step1 : G = MyBrightness);  // Increase Green
    (R > StepHold ? R -= Step1 : R = 0);                            // Decrease Red
  }
  else if (state1 == 3) {
    (B < MyBrightness - StepHold ? B += Step1 : B = MyBrightness);  // Increase Blue
    (G > StepHold ? G -= Step1 : G = 0);                            // Decrease Green
  }
