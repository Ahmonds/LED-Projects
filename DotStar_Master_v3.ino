...
float R = 0, G = 0, B = 0, r = 0, g = 0, b = 0;
uint32_t PastMillis = 0, MyColor = 0x00000000;
...
void Debounce() {
  ...
  if (ButtonStateB) {
    ...
    if (millis() - PrevBounceMillisB > dbDelayB) {
      ...
      if (ColorMode <= numPatterns) CircuitPlayground.strip.setPixelColor(ColorMode, MyR, MyG, MyB);
      else CircuitPlayground.strip.clear(); 
